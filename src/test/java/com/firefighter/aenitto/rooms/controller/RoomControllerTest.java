...
class RoomControllerTest {
	...
	@DisplayName("방 생성 - 실패 (날짜가 올바르게 설정되지 않은 경우)")
	@Test
	void createRoom_fail_invalid_customDate() throws Exception {
		...
		perform
			...
			.andDo(document("방 생성 - 실패 (시작일이 올바르게 설정되어 있지 않음)",
				preprocessResponse(prettyPrint()),
				responseFields(
	...
	@DisplayName("방 생성 -> 실패 (제약조건을 지키지 않은 경우)")
	@Test
	void createRoomFail() throws Exception {
		...
		perform.andDo(document("방 생성 - 실패 (제약 조건을 지키지 않은 경우)",
			preprocessResponse(prettyPrint()),
			responseFields(
				fieldWithPath("message").description("메시지"),
				fieldWithPath("status").description("상태 코드"),
