...
public class AuthControllerTest {
	...
	void signUplogin_fail_Invalid_identityToken() throws Exception {
		...
		mockMvc.perform(
			...
			.andDo(document("회원가입 및 로그인V1 - 실패 (유효하지 않은 identityToken)",
				preprocessResponse(prettyPrint()),
				responseFields(
					fieldWithPath("message").description("ACCESS TOKEN"),
					fieldWithPath("status").description("재발급 토큰"),
					fieldWithPath("timestamp").description("ACCESS TOKEN"),
					fieldWithPath("errors").description("ACCESS TOKEN")
